...
interface RegionEditorProps {
  ...
  /** Regions are in real pixels relative to the batch-cropped image dimensions below */
  croppedWidth: number;
...
export function RegionEditor({
  ...
  const getRelativePos = useCallback(
    (e: React.MouseEvent | MouseEvent) => {
      ...
      return {
        x: clampDisplay(e.clientX - rect.left, displaySize.w),
        y: clampDisplay(e.clientY - rect.top, displaySize.h),
      };
    },
    [displaySize]
  );
  ...
  useEffect(() => {
    ...
    const onUp = () => {
      ...
      // Only register if the region is at least 5 display px in each dimension
      if (wPx >= 5 && hPx >= 5) {
  ...
  // Display coords for each saved region
  ...
  const regionColor = "hsl(120 70% 55%)"; // green tint distinct from batch-crop blue
